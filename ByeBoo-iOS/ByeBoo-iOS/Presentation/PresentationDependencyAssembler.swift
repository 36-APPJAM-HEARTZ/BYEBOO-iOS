...
struct PresentationDependencyAssembler: DependencyAssembler {
    ...
    func assemble() {
        ...
        guard let getUserNameUseCase = DIContainer.shared.resolve(
            type: GetUserNameUseCase.self
        ),
              let fetchUserJourneyUseCase = DIContainer.shared.resolve(
                type: FetchUserJourneyUseCase.self
              ),
              let isForbiddenWordUseCase = DIContainer.shared.resolve(
                type: IsForbiddenWordUseCase.self
              )
        else {
        ...
        DIContainer.shared
            .register(type: WriteQuestionTypeViewModel.self) { container in
                guard let getQuestInfoUseCase = container.resolve(
                    type: GetQuestInfoUseCase.self
                ),
                      let saveQuestTypeUseCase = container.resolve(
                        type: SaveQuestTypeUseCase.self
                      ),
                      let editQuestTypeUseCase = container.resolve(
                        type: EditQuestTypeUseCase.self
                      ),
                      let isValidQuestAnswerUseCase = container.resolve(
                        type: IsValidQuestAnswerUseCase.self
                      ),
                      let saveCommonQuestUseCase = container.resolve(
                        type: SaveCommonQuestUseCase.self
                      ),
                      let isForbbidenWordUseCoase = container.resolve(
                        type: IsForbiddenWordUseCase.self
                      )
                else {
                    ByeBooLogger.error(ByeBooError.DIFailedError)
        ...
        DIContainer.shared
            .register(type: WriteActiveTypeViewModel.self) { container in
                guard let getQuestInfoUseCase = container.resolve(
                    type: GetQuestInfoUseCase.self
                ),
                      let saveActiveTypeUseCase = container.resolve(
                        type: SaveQuestActiveUseCase.self
                      ),
                      let saveQuestTypeUseCase = container.resolve(
                        type: SaveQuestTypeUseCase.self
                      ),
                      let editActiveQuestType = container.resolve(
                        type: EditQuestActiveUseCase.self
                      ),
        ...
        DIContainer.shared
            .register(type: InformationViewModel.self) { container in
                guard let sendUserUseCase = container.resolve(
                    type: SendUserUseCase.self
                ),
                      let checkValidNicknameUseCase = container.resolve(
                        type: CheckValidNicknameUseCase.self
                      )
                else {
                    ByeBooLogger.error(ByeBooError.DIFailedError)
                    return
        ...
        DIContainer.shared
            .register(type: ArchiveQuestViewModel.self) { container in
        ...
        DIContainer.shared.register(type: HomeViewModel.self) { container in
            guard let characterUseCase = container.resolve(
                type: FetchCharacterDialogueUseCase.self
            ),
                  let questStatusUseCase = container.resolve(
                    type: FetchQuestStatusUseCase.self
                  ),
                  let setHelperUseCase = container.resolve(
                    type: SetHelperUseCase.self
                  ),
                  let getHelperUseCase = container.resolve(
                    type: GetHelperUseCase.self
                  )
            else {
                ByeBooLogger.error(ByeBooError.DIFailedError)
                return
        ...
        DIContainer.shared
            .register(type: QuestStartViewModel.self) { container in
                guard let startJourneyUseCase = container.resolve(
                    type: StartJourneyUseCase.self
                ),
                      let postNewJourneyUseCase = container.resolve(
                        type: FetchNewJourneyUseCase.self
                      )
                else {
                    ByeBooLogger.error(ByeBooError.DIFailedError)
                    return
        ...
        DIContainer.shared
            .register(type: ProgressingQuestsViewModel.self) { container in
                guard let progressingQuestsUseCase = container.resolve(
                    type: GetProgressingQuestsUseCase.self
                ),
        ...
        DIContainer.shared.register(type: MyPageViewModel.self) { container in
            guard let getUserNameUseCase = container.resolve(
                type: GetUserNameUseCase.self
            ),
                  let logoutUseCase = container.resolve(
                    type: LogoutUseCase.self
                  ),
                  let withdrawUseCase = container.resolve(
                    type: WithdrawUseCase.self
                  ),
                  let changeNotificationPermissionUseCase = container.resolve(
                    type: ChangeNotificationPermissionUseCase.self
                  ),
                  let checkHasEnterMyPageUseCase = container.resolve(
                    type: CheckHasEnterMyPageUseCase.self
                  ),
                  let checkAlarmEnabledUseCase = container.resolve(
                    type: CheckAlarmEnabledUseCase.self
                  )
            else {
                ByeBooLogger.error(ByeBooError.DIFailedError)
                return
        ...
        DIContainer.shared
            .register(type: LookBackJourneyViewModel.self) { container in
        ...
        DIContainer.shared
            .register(type: NewJourneyViewModel.self) { container in
                guard let getNewJourneyUseCase = container.resolve(
                    type: GetNewJourneyUseCase.self
                )
                else {
                    ByeBooLogger.error(ByeBooError.DIFailedError)
                    return
        ...
        DIContainer.shared
            .register(type: ModifyNicknameViewModel.self) { container in
                guard let checkValidNicknameUseCase = container.resolve(
                    type: CheckValidNicknameUseCase.self
                ),
                      let modifyNicknameUseCase = container.resolve(
                        type: ModifyNicknameUseCase.self
                      )
                else {
                    ByeBooLogger.error(ByeBooError.DIFailedError)
                    return
        ...
        DIContainer.shared.register(type: LoginViewModel.self) { container in
            guard let socialLoginUseCase = container.resolve(
                type: SocialLoginUseCase.self
            ),
                  let getIsRegisteredUseCase = container.resolve(
                    type: GetIsRegisteredUseCase.self
                  ),
                  let getUserIDUseCase = container.resolve(
                    type: GetUserIDUseCase.self
                  )
            else {
                ByeBooLogger.error(ByeBooError.DIFailedError)
                return
        ...
        DIContainer.shared
            .register(type: CompletedQuestsViewModel.self) { container in
        ...
        DIContainer.shared
            .register(type: FinishJourneyViewModel.self) { container in
        ...
        DIContainer.shared
            .register(type: CommonQuestViewModel.self) { container in
        ...
        DIContainer.shared
            .register(type: CommonQuestMyAnswerViewModel.self) { container in
        ...
        DIContainer.shared
            .register(type: CommonQuestBottomSheetViewModel.self) { container in
